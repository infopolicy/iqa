...
All source code files are saved as "qmd" files and can be sourced into R as a batch process or interactively. Data are updated annually by editing source files in [bin/](bin/), then sourcing create_csvs.R in [code/](code/).  The HTML file is updated by rendering IQA_annual_data_report.qmd. 

## Build and Deployment
The resulting report is output as an html file in [code/](code/). This is deployed as a github pages static page through a standard deployment workflow with github actions. 
...
The following section describes the structure and contents of categories of files located in [data/](data/).  All data files are automatically generated and are not to be manually edited. 
...
Summary of requesters submitting RFCs by agency (one file for each fiscal year, e.g., requestor_summary_table_FY19.csv for FY-2019). Data extracted from electronic submission (FY-2019 to present).  Data were not contained in the Reports to Congress. 

## RFC_REQUESTOR_TYPES.csv
Summary of requesters submitting RFCs by fiscal year (sum across all agencies using data noted in requestor summary tables).  
...
# Detailed Descriptions of Source Files
The following section describes the structure and contents of categories of files located in [bin/](bin/).  These files are manually updated with new data following each annual data call. 

## agency_urls_source.xlsx
Table of agency IQA URLs, collected from data calls and manual screening of department websites.  These links will continue to be updated. This is the source file for AGENCY_URLS.csv.  To update, add or change an agency URL in one of the three columns (IQA URL 1, IQA URL 2, IQA URL 3).  Do not create new columns.  To add a new agency or department, create a new row and follow the naming convention if agency is within a larger department (e.g.,  Department of Labor - Occupational Safety and Health Administration (DOL-OSHA)).  


## peer_review_source.xlsx
Tables of peer review summaries.  Data extracted from OIRA Reports to Congress (for FY-2003 to FY-2018), and from electronic submission (FY-2019 to present).  One tab for each fiscal year.  Note: formatting differences between fiscal years for agencies within large departments are still unresolved between FY18/FY19+ data.  Some columns contain mathematical operations to make consistent for data extraction.  This is the source file for PEER_REVIEWS.csv, HISA.csv, PEER_REVIEW_SUMMARY_TABLE_FYXX.csv, PR_FY_PLOT.csv, and HISA_FY_PLOT.csv.  To update this file with a new year of fiscal year data, create a new tab using same fiscal year notation (eg, FY26 for fiscal year 2026). Create a new blank worksheet containing the same row headers as the previous FY.  For agencies reporting, make sure to use the same "Agency" and "Agency (abbrv.)" notation as in prior fiscal year tabs (not doing so can risk duplicative agencies in the government-wide summary of data).      

## rfcs_resolutions_source.xlsx
Final resolutions (if applicable) of requests for corrections and reconsiderations resolved within each fiscal year. Resolutions are correction/no correction/partial correction/handled by other processes/pending.   Data extracted from OIRA Reports to Congress (for FY-2003 to FY-2018).  In counting how requests are ultimately resolved, OMB extracted data from graphical flowcharts and tallied data based on the final conclusive resolution (after appeals, if applicable).  For example, if an initial determination was “no correction”, and the requestor appealed and the agency later granted a “partial correction”, this was counted only as a “partial correction” since the aim is determining how requests are ultimately resolved.  If an initial agency determination was “partial correction” and the requestor appealed, and the agency denied the appeal, this would also count as a “partial correction”.  This is a source file for RFC_FINAL_RESOLUTION.csv.  This file is no longer updated after FY-2018. 

## rfc_resolution_by_agency_source.xlsx
Final resolutions (if applicable) of requests for corrections and reconsiderations resolved within each fiscal year.  It is organized by both FY (tabs) and agency (rows).  This file supersedes rfcs_resolutions_source.xlsx for data collected beginning FY-2019. To update this file with a new year of fiscal year data, create a new tab using same fiscal year notation (eg, FY26 for fiscal year 2026). Create a new blank worksheet containing the same row headers as the previous FY.

## pending_rfcs_source.xlsx and pending_rfrs_source.xlsx
For each fiscal year, OIRA Reports to Congress (for FY-2003 to FY-2018) provided tables of all RFCs and RFRs that were reported as unresolved from prior fiscal years, and how those were ultimately resolved (correction/no correction/partial correction/handled by other processes/pending).  Those data are presented here.  These are ultimately merged with data from rfcs_resolutions_source and rfr_agencies_source to determine a running count of how RFCs are ultimately resolved. The data are aggregated into RFC_FINAL_RESOLUTION.csv.  For the data collection FY-2019 to FY-2023, this granular level of detail (by fiscal year) was not collected due to the multi-year nature of the call.  OMB was only able to ask about the nature of pending RFCs/RFRs at the end of FY-2023. For all subsequent data collections, OMB asks agencies to report the final resolutions (and also separately, asks agencies about pending requests).  As a result, these files no longer need to be updated with new data.  

## rfc_agencies_source.xlsx
Table of RFCs recieved by each agency, by fiscal year.  Data extracted from OIRA Reports to Congress (for FY-2003 to FY-2018), and from electronic submission (FY-2019 to FY-2023). Please see earlier notes (under [Data and Reports Prior to Fiscal Year 2007](#Data-and-Reports-Prior-to-Fiscal-Year-2007)) regarding adjustments to the totals for earlier years. This is the source file for RFC_AGENCIES.csv. To add new data, create a new fiscal year column (e.g., FY26 for fiscal year 2026), and add the number of RFCs recieved for each agency (rows).  

## rfr_agencies_source.xlsx
Table of RFRs recieved by each agency, by fiscal year.  Data extracted from OIRA Reports to Congress (for FY-2003 to FY-2018), and from electronic submission (FY-2019 to FY-2023). Please see earlier notes (under [Data and Reports Prior to Fiscal Year 2007](#Data-and-Reports-Prior-to-Fiscal-Year-2007)) regarding adjustments to the totals for earlier years. This is the source file for RFR_AGENCIES.csv.  To add new data, create a new fiscal year column (e.g., FY26 for fiscal year 2026), and add the number of RFRs recieved for each agency (rows).  

## rfc_information_types_source.xlsx
Summary of the types of information challenged in RFCs by agency (one tab for each fiscal year).  Data from electronic submission (FY-2019 to present).  This is the source file for RFC_INFORMATION_TYPES.csv.  To update this file with a new year of fiscal year data, create a new tab using same fiscal year notation (eg, FY26 for fiscal year 2026). Create a new blank worksheet containing an exact copy of the rows and column headers.  Add numeric data for each agency.  If a new agency needs to be added that has not been used in prior years, please add the agency to all worksheets to maintain consistency in calculations and avoid errors.   

## rfc_requestors_source.xlsx
Summary of requesters submitting RFCs by agency (one tab for each fiscal year).  Data from electronic submission (FY-2019 to present). This is the source file for RFC_REQUESTOR_TYPES.csv.  To update this file with a new year of fiscal year data, create a new tab using same fiscal year notation (eg, FY26 for fiscal year 2026). Create a new blank worksheet containing an exact copy of the row and column headers.  Add numeric data for each agency.  If a new agency needs to be added that has not been used in prior years, please add the agency to all worksheets to maintain consistency in calculations and avoid errors.   
