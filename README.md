...
Summary of requestors submitting RFCs by agency (one tab for each fiscal year).  Data from electronic submission (FY-2019 to present). This is the source file for RFC_REQUESTOR_TYPES.csv.  To update this file with a new year of fiscal year data, create a new tab using same fiscal year notation (eg, FY26 for fiscal year 2026). Create a new blank worksheet containing an exact copy of the rows and column headers.  Add numeric data for each agency.  If a new agency needs to be added that has not been used in prior years, please add the agency to all worksheets to maintain consistency in calculations and avoid errors.   
