...
subtitle: "Reporting of Agency Compliance Data (2003-2024)"
...
##### Our Nation relies on the flow of timely, objective, and credible information to inform the decisions of governments, businesses, organizations, and individuals. The Information Quality Act (IQA) of 2000 <a href="https://www.govinfo.gov/content/pkg/PLAW-106publ554/pdf/PLAW-106publ554.pdf" target="_blank">(Section 515 of Public Law 106–554; H.R. 5658)</a>  directed the Office of Management and Budget (OMB) to issue government-wide guidelines that provide policy and procedural guidance to Federal agencies for ensuring and maximizing the quality of information disseminated by Federal agencies, including a process for requesting correction. These guidelines and related initiatives are described below:
...
ggplot(RFCs_and_RFRS_longer, aes(x= reorder(Agency, Value), y = Value, fill = Category )) + 
 ...
 scale_fill_manual(values = setNames(Stackedbar_palette, unique(RFCs_and_RFRs_FY_longer$Category)))
```
######  **Figure 2: Number of Requests for Correction (RFCs) and Reconsideration (RFRs) by agency (FY2003-FY2024)** <br> Data presented as a total count of all requests.  Only those requests determined by OMB to have been initiated by the Information Quality Act are included.  <a href="https://www.reginfo.gov/public/jsp/Utilities/AcronymName.myjsp" target="_blank">Click here for the current agency acronyms used in this report</a>. A CSV file of these data is available [here](../data/csvs/FIGURE-2-TABLE.csv){target="_blank"}.
<br>
...
#tabular representation of figure data
#re-ordering rfcs to be first, and sorting by number of RFCs

tab2<-select(RFCs_and_RFRS,Agency, RFCs, RFRs)
...
tab2 |>
  ...
  cols_align("left")|>
  tab_header(
    title = " Table 2: Number of Requests for Correction (RFCs) and Reconsideration (RFRs) by agency (FY2003-FY2024)"
  )
...
#### **Figure 3: Percent of Requests for Correction (RFCs) received by agency (FY2003-FY2024)**
```{r message = FALSE, warning = FALSE, echo=FALSE}
...
tab3<-RFC_by_agency_piechart_filtered
tab3$Percent<- round(tab3$Percent, 1)
tab3 |>
  ...
  cols_align("left")|>
  tab_header(
    title = " Table 3: Percent of Requests for Correction (RFCs) received by agency (FY2003-FY2024)"
  )
...
#### **Figure 4: Final Resolutions of Requests for Correction (FY2003-FY2024)**
...
tab4<-RFC_resolutions_pie_final
...
tab4 |>
  ...
  cols_align("left")|>
  tab_header(
    title = " Table 4: Final Resolutions of Requests for Correction (FY2003-FY2024)"
  )
...
#### **Figure 5: Organizations requesting corrections (FY2019-FY2024)**
...
tab5<-RFC_requestors_pie_final
tab5 |>
  arrange(desc(Percent)) |>
  gt()|>
   tab_options(table.align = "left", container.width = pct(40)) |>
  cols_align("left")|>
  tab_header(
    title = " Table 5: Organizations requesting corrections (FY2019-FY2024)"
  )

```

> As noted earlier, historical data may contain inaccuracies due to variations in workflows and recordkeeping between agencies over time. In addition to workflow differences prior to FY2005, data from FY2019-FY2023 were collected during a single data call using a modernized collection mode with an online survey instrument. Data collected beginning FY2024 continued using the new survey instrument, and resumed data calls on an annual basis. Some data presented here were only available beginning in FY2019, because these data were not contained in the previous Reports to Congress.


<br>


#### **Figure 6: Types of information subjected to Requests for Correction (FY2019-FY2024)**
```{r message = FALSE, warning = FALSE, echo=FALSE, fig.dim = c(8,6)}
...
tab6 |>
  ...
  cols_align("left")|>
  tab_header(
    title = " Table 6: Types of information subjected to Requests for Correction (FY2019-FY2024)"
  )
...
######  **Figure 7: Number of peer reviews completed by fiscal year** <br> Data presented as a total count over all agencies of peer reviews subject to the Peer Review Bulletin (Influential Scientific Information and Highly Influential Scientific Assessments). A CSV file of these data is available [here](../data/csvs/FIGURE-7-TABLE.csv){target="_blank"}. 
...
tab7<-select(HISAs_and_PRs, FY, ISI, HISA)
tab7$FY<-gsub("'","20",as.character(tab7$FY))
...
tab7 |>
  ...
  cols_align("left")|>
  tab_header(
    title = " Table 7: Number of peer reviews completed by fiscal year: Influential Scientific Information (ISI) and Highly Influential Scientific Assessments (HISA)"
  )
...
#### **Figure 8: Number of peer reviews completed  by agency (FY2003-FY2024)**
```{r message = FALSE, warning = FALSE, echo=FALSE, fig.dim = c(8,6)}
...
###### **Figure 8: Number of peer reviews completed  by agency (FY2003-FY2024)** <br> Data presented as a total count of all peer reviews subject to the Peer Review Bulletin. A CSV file of these data is available [here](../data/csvs/FIGURE-8-TABLE.csv){target="_blank"}.
...
tab8<-select(ISI_and_HISA_by_agency, Agency, ISI, HISA)
...
tab8 |>
  ...
  cols_align("left")|>
  tab_header(
    title = "Table 8: Number of peer reviews completed  by agency: Influential Scientific Information (ISI) and Highly Influential Scientific Assessments (HISA) "
  )

```

#### **Figure 9: Percent of peer reviews completed by agency (FY2003-FY2024)**
```{r message = FALSE, warning = FALSE, echo=FALSE}
#peer_reviews_by_fiscal_year
#HISA_by_fiscal_year
...
######  **Figure 9: Percent of peer reviews completed by agency (FY2003-FY2024)** <br> Data presented as a percent of  all peer reviews subject to the Peer Review Bulletin (ISI+HISA). A CSV file of these data is available [here](../data/csvs/FIGURE-9-TABLE.csv){target="_blank"}.
...
tab9<-PR_agency_piechart_filtered
tab9$Percent<-round(tab9$Percent, 1)
...
tab9 |>
  ...
  cols_align("left")|>
  tab_header(
    title = " Table 9: Percent of peer reviews completed by agency (FY2003-FY2024)"
  )

```


> Peer review data provided by agencies to OMB include additional information that are not yet presented here. However, those data were still extracted from the  Reports to Congress and obtained during the post-FY2019 data calls. 
>
...
one_column_urls |>
  gt()|>
  tab_header(
    title = " Table 10: Agency Information Quality Act Websites"
  )
